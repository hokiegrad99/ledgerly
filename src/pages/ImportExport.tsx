...
import { FileUp, Database, Download, ShieldCheck, AlertTriangle, Upload, History } from 'lucide-react';
...
import type { ImportMapping, ImportSession, Transaction } from '../domain/types';
import { applyRulesToTransaction } from '../domain/rules';

const FILE_TYPE_LABEL: Record<ImportSession['fileType'], string> = {
  csv: 'CSV / TSV',
  qfx: 'QFX / OFX',
  ofx: 'QFX / OFX',
};

/** REQ-023: recent import sessions, most recent first. */
function ImportHistory() {
  const { importSessions, accountById } = useApp();
  const sessions = useMemo(() => [...importSessions].sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, 5), [importSessions]);
  if (sessions.length === 0) return null;
  return (
    <Card>
      <div className="flex items-center gap-2 border-b border-slate-100 px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:border-slate-800 dark:text-slate-400">
        <History className="h-3.5 w-3.5" /> Recent imports
      </div>
      <CardBody>
          ...
          {sessions.map((s) => (
            <li key={s.id} className="flex flex-wrap items-center justify-between gap-2 py-2 first:pt-0 last:pb-0">
              <div className="min-w-0">
                <div className="truncate text-sm font-medium text-slate-900 dark:text-slate-100">{s.fileName}</div>
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  {FILE_TYPE_LABEL[s.fileType] ?? s.fileType}
                  {s.accountId ? ` · ${accountById(s.accountId)?.name ?? 'Deleted account'}` : ''}
                  {' · '}{formatDate(s.createdAt.slice(0, 10))}
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-1.5">
                <Badge tone="green">{s.importedRows} imported</Badge>
                {s.skippedDuplicates > 0 && <Badge tone="amber">{s.skippedDuplicates} duplicates skipped</Badge>}
                {s.totalRows > s.importedRows + s.skippedDuplicates && (
                  <Badge tone="neutral">{s.totalRows - s.importedRows - s.skippedDuplicates} other</Badge>
                )}
              </div>
            </li>
          ))}
